...
function Progress({ value, side }: ProgressProps) {
  return (
    <div className="w-full h-3 bg-nanando-grey rounded">
      <div
        className={`${
          side == "right" ? "bg-nanando-white" : "bg-reniala-red"
        } h-3 rounded`}
        style={{ width: value + "%" }}
      ></div>
    </div>
...
function Item({ name, value, side, data }: ItemProps) {
  return (
    <div
      className={`absolute flex item-between justify-between w-full gap-x-14 gap-y-8 flex-wrap py-8 ${
        side == "right" ? "" : "bg-nanando-white text-nanando-black rounded-lg"
      }`}
      style={{
        clipPath:
          side == "right"
    ...
    >
      <span
        className={`${PoppinsBold.className} text-4xl w-full text-center md:text-3xl lg:text-5xl md:absolute md:top-1/2 md:left-1/2 text-nanando-soft-grey md:-translate-y-1/2 md:-translate-x-1/2 -z-10 `}
      >
        <div className="hidden md:block">
          <span
            className={`${side == "right" ? "invisible" : ""} ${
              value <= 40 ? "hidden" : ""
            }`}
          >
            {name}
        ...
        <p>
          <span className="md:hidden">{name}-</span>end
        </p>
      </span>
      {data.map((skill) => (
        <Skills key={skill.name} data={skill} side={side} />
      ))}
    </div>
  );
}
      ...
      <div className="flex flex-col w-full items-center justify-center pt-16">
        ...
        <div className="flex items-center justify-between md:justify-center flex-wrap gap-4 md:gap-10">
          ...
          {data.additionalSkillsData.noPhoto.map((item: any, index: number) => (
            <span
              key={index}
              className={`${PoppinsSemiBold.className} text-center bg-nanando-white rounded-md text-nanando-black p-2 uppercase flex align-items-center max-h-8 hover:scale-110`}
            >
              {item.name}
            </span>
          ))}
